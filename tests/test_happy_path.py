...
def storage_service_instance(docker_services):
    ...
    docker_services.wait_for_service("ftpd_server", 21, check_server=ftp_checker)

    return {'root_dir': root_dir,
            'root_url': '/api/v1/files/'}
...
def mocked_conf(monkeypatch):
    monkeypatch.setattr(settings, 'DEFAULT_FILE_STORAGE',
                        'storages.backends.ftp.FTPStorage', raising=False)
    monkeypatch.setattr(settings, 'FTP_STORAGE_LOCATION',
                        'ftp://pyconfr:pyconfr@localhost:21', raising=False)
